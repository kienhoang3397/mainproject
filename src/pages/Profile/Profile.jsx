...
const schemaWithoutCurrentPassword = yup.object().shape({
  ...
  phonenumber: yup
    .string()
    .matches(/^\d{9}$/, "Phone number must have exactly 8 digits"),
  email: yup.string().email("Invalid email format"),
});

const schemaWithCurrentPassword = yup.object().shape({
  ...schemaWithoutCurrentPassword.fields,
...
function Profile() {
  ...
  const [imgSrc, setImgSrc] = useState(
    "https://t4.ftcdn.net/jpg/02/19/17/07/360_F_219170739_FmBbreSQRvr23mLnpoenOzxIQbOwKnUP.jpg"
  );
  const user = useSelector((state) => state.infoUserApi.user);
  ...
  const onSubmit = async (data) => {
    try {
      ...
      setIsSubmitted(true);
      await updateUser(newInfoUser, user?._id, dispatch);
  ...
  return (
    <div className={styles.container}>
      <section className={styles.alert}>
        ...
        {error &&
          !isSubmitted &&
          renderAlert("error", "Error", error, "error-alert")}
      </section>
        ...
        <section className={styles.accountSetting}>
          <main className={styles.main}>
            <div className={styles.imgMainField}>
              <img className={styles.imgMain} src={imgSrc} alt="" />
            </div>
